...
def save_trained_model_as_json(model, filename="michal_v1.json"):
  model.save_model(filename)
  print(f"Model saved successfully as JSON: {filename}")
  ...
  for item in base_paths_config:
    ...
    for label, folders in label_folder_map.items():
      for folder in folders:
        ...
        files = glob.glob(os.path.join(folder_path, '*.wav')) + glob.glob(
...
def analyze_model_errors(y_test, custom_preds, scenarios_test):
  ...
  scenario_stats = defaultdict(
      lambda: {'total_bg': 0, 'total_drone': 0, 'fa_count': 0, 'md_count': 0}
  )
  ...
  header = (
      f"{'📋 DATASET (SCENARIO)':<{scen_w}} | {'TOTAL BG':<{bg_w}} |"
      f" {'FALSE ALARMS (FA)':<{fa_w}} | {'TOTAL DRONE':<{dr_w}} |"
      f" {'MISSED DET. (MD)':<{md_w}}"
  )
  print(header)
  print('-' * len(header))
  ...
  for scenario, stats in sorted(scenario_stats.items()):
    ...
    print(
        f'{scenario:<{scen_w}} | {tot_bg:<{bg_w}} | {fa_str:<{fa_w}} |'
        f' {tot_dr:<{dr_w}} | {md_str:<{md_w}}'
    )
  print('=' * len(header) + '\n')


def plot_scenarios_timeline_by_recordings(
    y_test,
    y_probs,
    custom_preds,
    scenarios_test,
  ...
  print('\n' + '=' * 80)
  print(
      '🎯 LOGGING FALSE ALARM TIMESTAMPS FOR AUDACITY INSPECTION (MATCHED'
      ' 100%):'
  )
  print('=' * 80)
  ...
  for scenario_name, recordings in sorted(nested_data.items()):
    ...
    fig.suptitle(
        f'🎬 EXPERIMENT TIMELINE: {scenario_name}',
        fontsize=14,
        fontweight='bold',
        y=0.98,
    )
    ...
    for idx, (rec_name, windows) in enumerate(sorted(recordings.items())):
      ...
      ax.plot(
          times_sec, y_values, color='gray', linestyle='--', alpha=0.2, lw=1.0
      )
      ax.plot(
          times_sec,
          probs,
      ...
      for i in range(len(windows)):
        ...
        if col == '#d62728':
          ...
          print(
              f'  ❌ [FA] File: {rec_name:<35} | Audacity Time:'
              f' {time_str} ({current_time:.2f}s)'
          )

      ax.set_title(
          f'🎵 Recording File: {rec_name}',
          fontsize=10,
          fontweight='bold',
          color='navy',
          loc='left',
      ...
      ax.set_yticks([0, 1])
      ax.set_yticklabels(
          ['Background (0)', 'Drone (1)'], fontsize=9, fontweight='bold'
      )
    ...
    save_path = os.path.join(
        output_dir, f'new_test_timeline_{scenario_name}.png'
    )
...
DATA_DIRECTORIES = [
    ...
    {
        'name': 'tut',
        'path': r'/Users/deviceone/Documents/data/tut/raw_extracted_segments',
    },
    {
        'name': 'ESC-50',
        'path': (
    ...
    },
    {
        'name': 'kakadoo_train',
        'path': (
            r'/Users/deviceone/Documents/data/2026.06.09_kakadoo_SPLITTED/train_set'
        ),
    },
    {
        'name': 'manatees_train',
        'path': (
            r'/Users/deviceone/Documents/data/2026.06.07_manatees/SPLITTED/train_set'
        ),
    },
    {
        'name': 'swan',
        'path': (
            r'/Users/deviceone/Documents/data/2026.06.17_swan/SPLITTED/train_set'
        ),
    },
    {
        'name': '2026.05.18_ostrich_attack_runs',
        'path': (
            r'/Users/deviceone/Documents/data/2026.05.18_ostrich_attack_runs/raw_extracted_segments'
        ),
    },
    {
        'name': '2026.06.02_snake',
        'path': (
            r'/Users/deviceone/Documents/data/2026.06.02_snake/raw_extracted_segments'
        ),
    },
    {
        'name': '2026.05.06_nautilus_1_snake',
        'path': (
            r'/Users/deviceone/Documents/data/2026.05.06_nautilus_1_snake/raw_extracted_segments'
        ),
    },
    {
        'name': '',
        'path': (
            r'/Users/deviceone/Documents/data/2026.05.13_nautilus_2_snake/raw_extracted_segments'
        ),
    },
]
...
if __name__ == '__main__':
  X_bin, y_bin, file_paths, scenarios_bin, timestamps_bin = prepare_data_raw(
      DATA_DIRECTORIES, binary_map
  )
  ...
  for scenario in unique_scenarios:
    ...
    if len(scen_files) >= 2:
      train_files, test_files = train_test_split(
          scen_files, test_size=0.20, random_state=42
      )
    else:
  ...
  print(
      f'--- Training Robust Model (fmax=2000Hz) on {len(X_train)} windows...'
      ' ---'
  )
  ...
  output_json_path = os.path.join(MODEL_OUTPUT_DIR, 'michal_v1.json')
  save_trained_model_as_json(model_bin, output_json_path)

  print(f'\n📌 Librosa Version used for training: {librosa.__version__}')